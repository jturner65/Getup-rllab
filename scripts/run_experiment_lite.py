...
def run_experiment(argv):
    ...
    if (args.resume_from is not None) and ('&|&' in args.resume_from) :#separate string on &|& to get iters and file location
        vals = args.resume_from.split('&|&')#dirRes | numItrs to go | new batchSize
        dirRes = vals[0]
        numItrs = int(vals[1])
        if (len(vals) > 2):
        ...
        if (len(vals) > 2):
            ...
            print('algo iters : {} cur iter :{} oldBatchSize : {} newBatchSize : {}'.format(algo.n_itr, algo.current_itr,oldBatchSize,algo.batch_size))
        else :
            print('algo iters : {} cur iter :{} '.format(algo.n_itr, algo.current_itr))
        algo.train()
    else:
